...
export function AvatarSelector({ selectedAvatarId, onAvatarChange, className }: AvatarSelectorProps) {
  const { t } = useLanguage()
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <div className={`relative group cursor-pointer ${className}`}>
          <Avatar className="h-24 w-24 border-4 border-slate-700 group-hover:border-cyan-500 transition-colors">
              ...
              >
                <div className="flex flex-col items-center space-y-2">
                  <Avatar className="h-16 w-16">
                    ...
                    <AvatarFallback className="text-lg bg-gradient-to-br from-cyan-500 to-blue-600">
                      {avatar.nickname.substring(0, 2)}
                    </AvatarFallback>
                  </Avatar>
                  <span className="text-sm font-medium text-center text-slate-300 truncate w-full">
                    {avatar.nickname}
                  </span>
                </div>
